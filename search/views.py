...
def search_results(request):
    ...
    if len(q) >= 2: # only if the cleaned query string length > 2 symbols, perform the DB searches: ----
        ...
        genes = GeneAnnotations.objects.filter(Q(gene__icontains = q) | Q(annotation__icontains = q)).values()
        ...
        if not genes_pd.empty:
            ...
            genes_pd["ratio_annotation"] = genes_pd.apply(lambda row: algorithims.levenshtein(row['annotation'], q), axis=1)
            # Sort the results by the Levenstein distance: ----
            genes_pd = genes_pd.sort_values(by=['ratio_gene', 'ratio_annotation'], ascending=False)
            # Remove columns that are not needed in the final results: ----
            del genes_pd["ratio_gene"]
            del genes_pd["ratio_annotation"]
