...
def create_datatables_api(mongo_aggregate, request_get, select_pipeline, out_keys, as_list=True):
  ...
  pipeline = (select_pipeline + filter_pipeline + [
    {"$facet": {
      "info": [{"$count": "filtered"}],
...
def create_datatables_api_mongodb(mongo_aggregate, request_get, select_pipeline, out_keys, as_list=True): # Does not work on Azure Cosmos MongoDB (vCore) 7.0
  draw = int(request_get["draw"])
  start = int(request_get["start"])
  length = int(request_get["length"])
