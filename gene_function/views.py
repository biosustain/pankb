...
def gene_info(request):
    ...
    gene_info_json = json.dumps(
        gene_info, default=str
    )  # json dumps replaces the single quotes with the double ones
...
def download_gene_info_table_csv(request):
    ...
    gene_info = GeneInfo.objects.find(
        filter_params, fields, sort=[("start_position", 1), ("end_position", 1)]
    )
...
def aa_pos_overview(request):
    ...
    url2 = (
        ...
        + "species/"
        + species
        + "/panalleleome/gene_data/"
        + gene
        + "/"
        + gene
        + "_pan_aa_thresh_core_dom_var_pos.csv"
    ...
    # Compose a context for the template rendering: ----
    context = {"num_genomes": num_genomes, "dataset": json.dumps(dataset_dict2)}
    return HttpResponse(template.render(context, request))
...
def msa(request):
    ...
    if r2.status_code == requests.codes.ok:
        ...
        r2_text = ""
    # Compose a context for the template rendering: ----
    context = {"AAData": json.dumps(json_obj1), "msaData": json.dumps(r2_text)}
    return HttpResponse(template.render(context, request))
...
def genome_info(request):
    ...
    context = {
        ...
        "antismash_url": (
            ""
            if not genome_info_dict.get("antismash_url", False)
            else genome_info_dict["antismash_url"]
        ),
    }
    return HttpResponse(template.render(context, request))
...
def pathway_info(request):
    ...
    try:
        pathway_info = PathwayInfo.objects.find_one(
            {"pathway_id": pathway_id}, {"_id": 0}
        )
...
def pathway_gene_annotation_json(request):
    ...
    select_pipeline = [
        ...
        {
            "$project": {gk: 1 for gk in gene_keys}
        },  # This projection keeps document size smaller
    ]
