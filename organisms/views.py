...
def organisms(request):
    ...
    organisms = [
        (
            [org[k] for k in org_keys]
            + [int(org["pangenome_analysis"] in HIGHLIGHTED_SPECIES)]
        )
        for org in organisms
    ]
    organisms_json = json.dumps(
        organisms, default=str
    )  # json dumps replaces the single quotes with the double ones
    # Compose the render context: ----
    context = {"dataset": organisms_json}
...
def download_organisms_table_csv(request):
    ...
    downloaded_file_name = (
        "Organisms__"
        + (f"{family}__" if family else "")
        + time.strftime("%Y-%m-%d_%H-%M")
        + ".csv"
    )

    response = csv_export.dict_writer_response(
        downloaded_file_name, org_keys, organisms
    )
    return response
